...
def create_pdf_report(result, confidence, abnormal_prob, normal_prob, recommendation):
    buffer = BytesIO()
    ...
    pdf.drawString(50, y, "Diabetic Retinopathy Screening Report")

    y -= 45
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Result:")
    pdf.setFont("Helvetica", 12)
    pdf.drawString(170, y, result)

    y -= 28
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Confidence:")
    pdf.setFont("Helvetica", 12)
    pdf.drawString(170, y, f"{confidence:.1f}%")

    y -= 28
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Abnormal Probability:")
    pdf.setFont("Helvetica", 12)
    pdf.drawString(220, y, f"{abnormal_prob:.1f}%")

    y -= 28
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Normal Probability:")
    pdf.setFont("Helvetica", 12)
    pdf.drawString(220, y, f"{normal_prob:.1f}%")

    y -= 45
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Recommended Action:")
    y -= 22
    ...
    y -= 95
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Generated At:")
    pdf.setFont("Helvetica", 11)
    pdf.drawString(170, y, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"))
    ...
    disclaimer = (
        "This report is generated by an AI-assisted screening tool. "
        "It is not a medical diagnosis and should be reviewed by a qualified healthcare professional."
    )
    text = pdf.beginText(50, y)
    text.setLeading(14)
    for line in [disclaimer[:90], disclaimer[90:]]:
        text.textLine(line.strip())
    pdf.drawText(text)
...
try:
    ...
    if not uploaded:
        ...
        with right:
            ...
            recommendation = (
                "Please consult an eye-care professional for clinical evaluation."
                if label == "Abnormal"
                else "Continue routine screening as advised by a healthcare professional."
            )

            pdf_report = create_pdf_report(
                result=friendly_label,
                confidence=confidence,
                abnormal_prob=float(probs[0] * 100),
                normal_prob=float(probs[1] * 100),
            ...
            st.download_button(
                "Download PDF report",
                data=pdf_report,
                file_name="diabetic_retinopathy_screening_report.pdf",
